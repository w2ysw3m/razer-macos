...
5. 发行打包：已签名并公证的原生 app、hardened runtime 与清楚的 macOS 权限说明。

发行打包是 release-only 工作。它不是缺少的设备控制功能，也不是缺少的 Launch at Login 功能；使用 `./script/build_and_run.sh` 做本地开发不需要完成 Packaging。
...
已打包、已签名、已公证的原生 build 尚未完成，因为它需要 release artifacts、签名身份、hardened runtime 验证与 Apple notarization credentials。本地开发请用 `./script/build_and_run.sh` 从 source 运行。

legacy Electron packaging flow 仍支持 ad-hoc signing：
